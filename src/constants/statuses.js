export const statusMetadata = {
  new_request: {
    label: "New request",
    color: "primary",
    description: "New appointment request awaiting contact.",
  },
  new: { label: "New", color: "primary" },
  under_review: { label: "Under review", color: "info" },
  waiting_client_reply: { label: "Waiting client reply", color: "warning" },
  requested: { label: "Requested", color: "primary" },
  active: { label: "Active", color: "success" },
  Active: { label: "Active", color: "success" },
  available: { label: "Available", color: "success" },
  Available: { label: "Available", color: "success" },
  limited: { label: "Limited", color: "warning" },
  Limited: { label: "Limited", color: "warning" },
  offline: { label: "Offline", color: "danger" },
  inactive: { label: "Inactive", color: "danger" },
  Inactive: { label: "Inactive", color: "danger" },
  maintenance: { label: "Maintenance", color: "warning" },
  "On Duty": { label: "On Duty", color: "success" },
  "Off Duty": { label: "Off Duty", color: "neutral" },
  "On Leave": { label: "On Leave", color: "warning" },
  Invited: { label: "Invited", color: "warning" },
  Suspended: { label: "Suspended", color: "danger" },
  Booked: { label: "Booked", color: "info" },
  Unavailable: { label: "Unavailable", color: "danger" },
  "Emergency Cover": { label: "Emergency Cover", color: "danger" },
  "Pending coverage": { label: "Pending coverage", color: "warning" },
  "Pending approval": { label: "Pending approval", color: "warning" },
  critical: { label: "Critical", color: "danger" },
  contacted: { label: "Contacted", color: "info" },
  confirmed: { label: "Confirmed", color: "info" },
  rescheduled: { label: "Rescheduled", color: "warning" },
  checked_in: { label: "Checked in", color: "success" },
  waiting: { label: "Waiting", color: "warning" },
  waiting_for_doctor: { label: "Waiting for doctor", color: "warning" },
  waiting_doctor: { label: "Waiting for doctor", color: "warning" },
  waiting_reception: { label: "Waiting at reception", color: "warning" },
  in_consultation: { label: "In consultation", color: "info" },
  soap_in_progress: { label: "SOAP in progress", color: "info" },
  prescription_pending: { label: "Prescription pending", color: "warning" },
  lab_requested: { label: "Lab requested", color: "warning" },
  follow_up_needed: { label: "Follow-up needed", color: "warning" },
  lab_pending: { label: "Lab pending", color: "warning" },
  imaging_pending: { label: "Imaging pending", color: "warning" },
  pharmacy_pending: { label: "Pharmacy pending", color: "warning" },
  billing_pending: { label: "Billing pending", color: "warning" },
  doctor_review_pending: { label: "Doctor review pending", color: "warning" },
  completed: { label: "Completed", color: "success" },
  cancelled: { label: "Cancelled", color: "danger" },
  no_show: { label: "No show", color: "danger" },
  rejected: { label: "Rejected", color: "danger" },
  duplicate: { label: "Duplicate", color: "neutral" },
  draft: { label: "Draft", color: "neutral" },
  sent_to_pharmacy: { label: "Sent to pharmacy", color: "info" },
  dispensing_pending: { label: "Dispensing pending", color: "warning" },
  not_sent: { label: "Not sent", color: "neutral" },
  partially_dispensed: { label: "Partially dispensed", color: "warning" },
  dispensed: { label: "Dispensed", color: "success" },
  out_of_stock: { label: "Out of stock", color: "danger" },
  low_stock: { label: "Low stock", color: "warning" },
  near_expiry: { label: "Near expiry", color: "warning" },
  expired: { label: "Expired", color: "danger" },
  paid: { label: "Paid", color: "success" },
  recorded: { label: "Recorded", color: "info" },
  pending: { label: "Pending", color: "warning" },
  unpaid: { label: "Unpaid", color: "warning" },
  void: { label: "Void", color: "neutral" },
  deposit: { label: "Deposit", color: "info" },
  partial: { label: "Partial", color: "warning" },
  refunded: { label: "Refunded", color: "info" },
  sent_to_pos: { label: "Sent to POS", color: "info" },
  manually_billed: { label: "Manually billed", color: "neutral" },
  follow_up_due: { label: "Follow-up due", color: "warning" },
  pending_payment: { label: "Pending payment", color: "warning" },
  allergy: { label: "Allergy", color: "warning" },
  chronic_condition: { label: "Chronic condition", color: "warning" },
  aggressive: { label: "Aggressive", color: "danger" },
  senior_pet: { label: "Senior pet", color: "info" },
  due_today: { label: "Due today", color: "warning" },
  due_soon: { label: "Due soon", color: "info" },
  overdue: { label: "Overdue", color: "danger" },
  scheduled: { label: "Scheduled", color: "info" },
  missed: { label: "Missed", color: "danger" },
  normal: { label: "Normal", color: "success" },
  urgent: { label: "Urgent", color: "warning" },
  visit: { label: "Visit", color: "info" },
  lab: { label: "Lab", color: "warning" },
  vaccination: { label: "Vaccination", color: "success" },
  prescription: { label: "Prescription", color: "info" },
  surgery: { label: "Surgery", color: "warning" },
  imaging: { label: "Imaging", color: "info" },
  vaccine: { label: "Vaccine", color: "info" },
  deworming: { label: "Deworming", color: "warning" },
  none: { label: "None", color: "neutral" },
};

export const STATUS = Object.fromEntries(
  Object.keys(statusMetadata).map((status) => [status.toUpperCase(), status]),
);

export const getStatusMeta = (status) =>
  statusMetadata[status] || {
    label: String(status || "Unknown").replaceAll("_", " "),
    color: "neutral",
  };
